...
class Task:
    """
    TASK: Help Turtle to visit his friend, Mickey.
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            angle = randint(0,360)
            mickey_x = cos(radians(angle))*distance
            mickey_y = sin(radians(angle))*distance
        else:
            mickey_x = int(input("Enter Mickey's x: "))
            mickey_y = int(input("Enter Mickey's y: "))
        self.mickey = Home(mickey_x,mickey_y,32,32,IMG_HOME,"Mickey")
        stage.add_object(self.mickey)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Mickey's home is at ({self.mickey.x:.2f},{self.mickey.y:.2f})")
